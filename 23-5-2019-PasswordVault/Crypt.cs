...
namespace PasswordVault
{
    public static class Crypt
    {
        public static string EncryptString(string plainText, string key, string iv)
        {
            ...
            // Set key and IV
            rijndaelCipher.Key = ConvertToAcceptableKeyByteArray(key, 128);
            rijndaelCipher.IV = ConvertToAcceptableKeyByteArray(iv, rijndaelCipher.BlockSize);
        ...
        private static byte[] ConvertToAcceptableKeyByteArray(string key, int bitLen)
        {
            // make a list of bytes large enough by adding the key each time its not big enough
            List<byte> bytes = new List<byte>();
            while (bytes.Count < bitLen / 8)
            {
            ...
            // make sure it  is not to big....
            while (bytes.Count > bitLen / 8) bytes.RemoveAt(0);
            // return it as an byte array
